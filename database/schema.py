...
SCHEMA_STATEMENTS = [
    ...
    CREATE TABLE IF NOT EXISTS case_timeline_events (
        ...
        created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (child_id) REFERENCES missing_children(id) ON DELETE CASCADE,
        FOREIGN KEY (report_id) REFERENCES found_child_reports(id) ON DELETE SET NULL
    )
    """,
    """
    ...
    "CREATE INDEX IF NOT EXISTS idx_case_timeline_child ON case_timeline_events(child_id)",
    "CREATE INDEX IF NOT EXISTS idx_case_timeline_case_id ON case_timeline_events(case_id)",
    "CREATE INDEX IF NOT EXISTS idx_case_timeline_created_at ON case_timeline_events(created_at)",
    ...
    "CREATE INDEX IF NOT EXISTS idx_police_station_cache_key ON police_station_cache(cache_key)",
...
def initialize_database() -> None:
    ...
    try:
        ...
        _migrate_location_columns(connection)
        connection.commit()
...
def _migrate_missing_children_age_range(connection) -> None:
    ...
    legacy_columns = {
        str(column["name"])
        for column in connection.execute("PRAGMA table_info(missing_children)").fetchall()
    }
    legacy_latitude_expression = "last_seen_latitude" if "last_seen_latitude" in legacy_columns else "NULL"
    legacy_longitude_expression = "last_seen_longitude" if "last_seen_longitude" in legacy_columns else "NULL"
    ...
    connection.execute(
        ...
        SELECT
            ...
            identification_marks,
            last_seen_location,
            {legacy_latitude_expression},
            {legacy_longitude_expression},
            last_seen_date,
            last_seen_time,
            description,
            status,
...
def _migrate_location_columns(connection) -> None:
    _add_column_if_missing(connection, "missing_children", "last_seen_latitude", "REAL")
    _add_column_if_missing(connection, "missing_children", "last_seen_longitude", "REAL")
    _add_column_if_missing(connection, "found_child_reports", "found_latitude", "REAL")
    _add_column_if_missing(connection, "found_child_reports", "found_longitude", "REAL")
...
def _add_column_if_missing(connection, table_name: str, column_name: str, column_definition: str) -> None:
    ...
    if not columns or column_name in columns:
        return
    logger.info("Adding %s.%s column for location mapping", table_name, column_name)
