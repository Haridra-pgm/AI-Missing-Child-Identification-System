...
def create_found_child_report(connection: sqlite3.Connection, report_data: dict[str, Any]) -> int:
    cursor = connection.execute(
        ...
        (
            ...
            report_data.get("found_latitude"),
            report_data.get("found_longitude"),
...
def fetch_report_matches(connection: sqlite3.Connection, report_id: int) -> list[dict[str, Any]]:
    rows = connection.execute(
        ...
        SELECT
            ...
            child.last_seen_latitude,
            child.last_seen_longitude,
...
def _row_to_report(row: sqlite3.Row) -> dict[str, Any]:
    return {
        ...
        "found_latitude": row["found_latitude"],
        "found_longitude": row["found_longitude"],
...
def _row_to_match(row: sqlite3.Row) -> dict[str, Any]:
    return {
        ...
        "last_seen_latitude": row["last_seen_latitude"],
        "last_seen_longitude": row["last_seen_longitude"],
