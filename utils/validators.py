...
def validate_child_data(data: dict[str, Any]) -> dict[str, Any]:
    ...
    last_seen_latitude, last_seen_longitude = validate_optional_coordinates(
        data.get("last_seen_latitude"),
        data.get("last_seen_longitude"),
        latitude_field="Last seen latitude",
        longitude_field="Last seen longitude",
    )
...
def validate_optional_coordinates(
    latitude: Any,
    longitude: Any,
    *,
    ...
    latitude_text = _normalize_text(latitude)
    longitude_text = _normalize_text(longitude)

    if not latitude_text and not longitude_text:
        return None, None

    if not latitude_text or not longitude_text:
        raise ValidationError(f"Enter both {latitude_field.lower()} and {longitude_field.lower()}, or leave both blank.")

    parsed_latitude = _validate_coordinate_value(latitude_text, latitude_field, -90.0, 90.0)
    parsed_longitude = _validate_coordinate_value(longitude_text, longitude_field, -180.0, 180.0)
    return parsed_latitude, parsed_longitude


def _validate_coordinate_value(value: str, field_name: str, minimum: float, maximum: float) -> float:
    try:
