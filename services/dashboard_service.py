...
from database.repositories.dashboard_repository import (
    ...
    fetch_case_details,
    fetch_child_id_by_case_id,
    fetch_dashboard_statistics,
    fetch_filtered_child_records,
    fetch_recent_registrations,
    fetch_search_history,
)
from database.repositories.found_report_repository import fetch_reports_for_child_ids
...
def get_child_case_details(child_id: int) -> dict[str, Any]:
    ...
    try:
        ...
        with database_transaction() as connection:
            ...
            case_details["timeline"] = fetch_case_timeline(connection, normalized_child_id)
            case_details["matched_reports"] = fetch_reports_for_child_ids(connection, [normalized_child_id])
...
def get_child_case_details_by_case_id(case_id: str) -> dict[str, Any]:
    normalized_case_id = _optional_text(case_id, "Case ID", 80)
    ...
    try:
        initialize_database()
        with database_transaction() as connection:
            child_id = fetch_child_id_by_case_id(connection, normalized_case_id)
            if child_id is None:
                raise ValidationError("Selected case was not found.")
            case_details = fetch_case_details(connection, child_id)
            if case_details is None:
                raise ValidationError("Selected case was not found.")
            case_details["timeline"] = fetch_case_timeline(connection, child_id)
            case_details["matched_reports"] = fetch_reports_for_child_ids(connection, [child_id])
            create_activity_log(
                connection=connection,
                action="view_linked_case_details",
                entity_type="missing_child",
                entity_id=child_id,
                details={"case_id": normalized_case_id},
            )
        return case_details
    except ValidationError:
        raise
    except sqlite3.Error as exc:
        logger.exception("Database error while loading case details case_id=%s", normalized_case_id)
        raise DashboardDatabaseError("Unable to load linked case details.") from exc
