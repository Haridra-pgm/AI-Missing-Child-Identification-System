...
def _verify_dynamic_progression_strength(sample: dict[str, object]) -> None:
    ...
    target_stage_delta = _mean_generated_stage_delta(generated_images[0], generated_images[-1])
    if target_stage_delta <= 2.0:
        raise AssertionError(
            "Different target ages produced nearly identical generated estimates "
            f"(target-stage delta={target_stage_delta:.3f})."
        )
    if min(identity_scores) <= 0:
        raise AssertionError(f"Identity-preservation scoring failed: {identity_scores}.")

    metrics = ", ".join(
        f"target {target}: source_delta={delta:.2f}, identity={identity:.3f}"
        for target, delta, identity in zip(target_ages, deltas, identity_scores)
    )
    print(
        "OK: dynamic target-age progression validated "
        f"({metrics}; generated-stage delta={target_stage_delta:.2f})."
    )
...
def _mean_face_region_delta(source_image, generated_image, bbox: list[int] | tuple[int, int, int, int]) -> float:
    ...
    generated_region = generated_image.astype(np.float32)
    if source_region.shape[:2] != generated_region.shape[:2]:
        generated_region = cv2.resize(generated_region, (source_region.shape[1], source_region.shape[0]))
    return float(np.mean(np.abs(generated_region - source_region)))


def _mean_generated_stage_delta(first_generated, last_generated) -> float:
    first_region = first_generated.astype(np.float32)
    last_region = last_generated.astype(np.float32)
    if first_region.shape[:2] != last_region.shape[:2]:
        last_region = cv2.resize(last_region, (first_region.shape[1], first_region.shape[0]))
    return float(np.mean(np.abs(last_region - first_region)))
