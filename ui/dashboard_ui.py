...
from config.constants import GENDER_OPTIONS, ROLE_AUTHORITY, ROLE_PARENT
from services.dashboard_service import (
    ...
    get_child_case_details_by_case_id,
    get_dashboard_overview,
    search_dashboard_child_records,
)
from services.role_dashboard_service import RoleDashboardError, get_parent_dashboard
...
def render_linked_case_details_page(user: dict[str, Any], case_id: str) -> None:
    render_page_header(
        "Linked Case Details",
        "Case details opened from a ChildShield AI QR code.",
    )
    try:
        if user.get("role") == ROLE_AUTHORITY:
            case_details = get_child_case_details_by_case_id(case_id)
        elif user.get("role") == ROLE_PARENT:
            parent_data = get_parent_dashboard(user)
            case_details = next(
                (case for case in parent_data["cases"] if case["case_id"] == case_id),
                None,
            )
            if case_details is None:
                raise ValidationError("This parent account is not linked to the requested case.")
        else:
            st.warning("Only parent and authority users can open case QR details.")
            return
    except (ValidationError, DashboardDatabaseError, RoleDashboardError) as exc:
        st.error(str(exc))
        logger.info("Linked case details failed case_id=%s error=%s", case_id, exc)
        return
...
def _render_case_details(case_details: dict[str, Any]) -> None:
    ...
    with child_col:
        ...
        st.write(f"Last seen latitude: {_display_value(case_details.get('last_seen_latitude'))}")
        st.write(f"Last seen longitude: {_display_value(case_details.get('last_seen_longitude'))}")
    ...
    _render_case_location_map(case_details)
    _render_case_nearby_police_stations(case_details)
    _render_uploaded_images(case_details["images"])
    _render_embedding_status(case_details["embeddings"])
    render_case_qr_tools(case_details, key_prefix="admin_case")
    render_case_timeline(case_details.get("timeline"))
...
def _render_case_location_map(case_details: dict[str, Any]) -> None:
    if case_details.get("last_seen_latitude") is None or case_details.get("last_seen_longitude") is None:
        return

    st.markdown("**Last Seen Location Map**")
    render_location_map(
        last_seen_location=case_details.get("last_seen_location"),
        last_seen_latitude=case_details.get("last_seen_latitude"),
        last_seen_longitude=case_details.get("last_seen_longitude"),
        last_seen_timestamp=_last_seen_timestamp(case_details),
        key=f"admin_case_location_map_{case_details['child_id']}",
    )


def _render_case_nearby_police_stations(case_details: dict[str, Any]) -> None:
    location = _select_police_lookup_location(case_details)
    if location is None:
        return

    st.caption(f"Police station search location: {location['label']} - {location['name']}")
    render_nearby_police_stations(
        found_latitude=location["latitude"],
        found_longitude=location["longitude"],
        key_prefix=f"admin_case_{case_details['child_id']}_{location['key']}",
    )


def _select_police_lookup_location(case_details: dict[str, Any]) -> dict[str, Any] | None:
    for report in case_details.get("matched_reports") or []:
        if report.get("found_latitude") is not None and report.get("found_longitude") is not None:
            return {
                "key": f"found_report_{report['report_id']}",
                "label": "Found Location",
                "name": _display_value(report.get("location")),
                "latitude": report["found_latitude"],
                "longitude": report["found_longitude"],
            }
    ...
    return {
        ...
        "name": _display_value(case_details.get("last_seen_location")),
        "latitude": case_details["last_seen_latitude"],
        "longitude": case_details["last_seen_longitude"],
    }


def _last_seen_timestamp(record: dict[str, Any]) -> str | None:
    date_value = _display_value(record.get("last_seen_date"))
    time_value = _display_value(record.get("last_seen_time"))
