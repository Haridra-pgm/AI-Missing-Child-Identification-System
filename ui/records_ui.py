    ...
    for record in records:
        ...
        with st.expander(title):
            ...
            with child_col:
                ...
                st.write(f"Last seen latitude: {display_value(record.get('last_seen_latitude'))}")
                st.write(f"Last seen longitude: {display_value(record.get('last_seen_longitude'))}")
            ...
            _render_record_location_map(record)
...
def _render_record_location_map(record: dict[str, Any]) -> None:
    if record.get("last_seen_latitude") is None or record.get("last_seen_longitude") is None:
        return

    st.markdown("**Last Seen Location Map**")
    render_location_map(
        last_seen_location=record.get("last_seen_location"),
        last_seen_latitude=record.get("last_seen_latitude"),
        last_seen_longitude=record.get("last_seen_longitude"),
        last_seen_timestamp=_last_seen_timestamp(record),
        key=f"records_location_map_{record['child_id']}",
    )
